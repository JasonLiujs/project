...
import { WarshipDemoOverlay } from "./WarshipDemoOverlay";
import { createRenderer, GameRenderer } from "./graphics/GameRenderer";
import { GoToPlayerEvent, GoToUnitEvent } from "./graphics/TransformHandler";
...
export class ClientGameRunner {
  ...
  private warshipDemoOverlay: WarshipDemoOverlay | null = null;
  private focusedDemoWarshipID: number | undefined;
  ...
  public start() {
    ...
    if (
      this.lobby.gameStartInfo?.config.demoScenario === "warship-diagonal-chase"
    ) {
      this.warshipDemoOverlay = new WarshipDemoOverlay(
        this.gameView,
    ...
    this.worker.start((gu: GameUpdateViewData | ErrorUpdate) => {
      ...
      this.warshipDemoOverlay?.update(gu);
      const demoWarship = this.gameView.units(UnitType.Warship)[0];
      if (
        this.warshipDemoOverlay !== null &&
        demoWarship !== undefined &&
        demoWarship.id() !== this.focusedDemoWarshipID
      ) {
        this.focusedDemoWarshipID = demoWarship.id();
        this.renderer.transformHandler.scale = 10;
        this.eventBus.emit(new GoToUnitEvent(demoWarship));
      }
      this.renderer.tick();
  ...
  public stop() {
    ...
    this.warshipDemoOverlay?.dispose();
    this.warshipDemoOverlay = null;
